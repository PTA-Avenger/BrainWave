    ...
    public class UserRepository
    {
        ...
        public async Task<List<User>> GetUsersAsync()
        {
            ...
            cmd.CommandText = _provider == "postgres"
                ? @"SELECT ""userid"", ""f_name"", ""l_name"", ""email"", ""password_hash"", ""role"" 
                     FROM ""User"""
        ...
        public async Task<User?> GetUserByEmailAsync(string email)
        {
            ...
            using var cmd = conn.CreateCommand();
            cmd.CommandText = _provider == "postgres"
                ? @"SELECT ""userid"", ""f_name"", ""l_name"", ""email"", ""password_hash"", ""role"", ""profile_picture"" 
        ...
        public async Task<List<BrainWave.Api.DTOs.UserDtos>> GetFilteredUsersAsync(BrainWave.Api.DTOs.UserFilterDto? filter = null)
        {
            ...
            {
                whereClause = _provider == "postgres" ? @"WHERE ""role"" = @role" : "WHERE Role = @role";
            ...
            var orderBy = filter?.SortBy ?? "UserID";
            var direction = filter?.SortDescending == true ? "DESC" : "ASC";

            cmd.CommandText = _provider == "postgres"
                ? $@"SELECT ""userid"", ""f_name"", ""l_name"", ""email"", ""role"", ""profile_picture"" 
                       FROM ""User"" {whereClause} ORDER BY ""{orderBy.ToLower()}"" {direction}"
                : $@"SELECT UserID, F_Name, L_Name, Email, Role, Profile_Picture 
                       FROM Users {whereClause} ORDER BY {orderBy} {direction}";
