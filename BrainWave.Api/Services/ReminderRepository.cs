...
public class ReminderRepository
{
    ...
    public async Task<List<Reminder>> GetRemindersByUserIdAsync(int userId)
    {
        ...
        using var cmd = conn.CreateCommand();
        cmd.CommandText = _provider == "postgres"
            ? "SELECT r.\"reminderid\", r.\"taskid\", r.\"reminder_type\", r.\"notify_time\" FROM reminders r INNER JOIN tasks t ON r.\"taskid\" = t.\"taskid\" WHERE t.\"userid\" = @userId"
            : "SELECT r.ReminderID, r.TaskID, r.Reminder_Type, r.Notify_Time FROM Reminders r INNER JOIN Tasks t ON r.TaskID = t.TaskID WHERE t.UserID = @userId";
    ...
    public async Task<List<Reminder>> GetDueRemindersAsync()
    {
        ...
        using var cmd = conn.CreateCommand();
        cmd.CommandText = _provider == "postgres"
            ? "SELECT \"reminderid\", \"taskid\", \"reminder_type\", \"notify_time\" FROM reminders WHERE \"notify_time\" <= @now"
            : "SELECT ReminderID, TaskID, Reminder_Type, Notify_Time FROM Reminders WHERE Notify_Time <= @now";
    ...
    public async Task UpdateReminderAsync(Reminder reminder)
    {
        ...
        using var cmd = conn.CreateCommand();
        cmd.CommandText = _provider == "postgres"
            ? "UPDATE reminders SET \"reminder_type\" = @type, \"notify_time\" = @notifyTime WHERE \"reminderid\" = @id"
            : "UPDATE Reminders SET Reminder_Type = @type, Notify_Time = @notifyTime WHERE ReminderID = @id";
